...
public interface MenuRepository extends JpaRepository<MenuGenerado, Long> {
	...
	@Query(value = "select M from MenuDetalle M where M.idGenerado = (select max(G.idGenerado) from MenuGenerado G where G.idPersona = ?1)")
	List<MenuDetalle> buscarUltimoMenuGenerado(Long idPersona);
	
	@Query(value = "select M from MenuDetalle M where M.idGenerado in (select G.idGenerado from MenuGenerado G where G.idPersona = ?1 and fe_generado >= ?2)")
